...
**ガチャに出すバナーは必ず選ぶこと。** キャラ定義を足しただけでは、どのガチャからも
出ない。`data/gacha/banners.json` のキャラバナーはすべて `pool` にIDを明示しており、
そこに入っていないキャラは抽選対象にならないため。ツールの「このキャラを出すガチャ」で
選んだバナーへ `gachaPools` として追記される(既定は常設キャラ召喚)。
ダンジョンのキャラドロップは `limited !== true` の全キャラが対象なので、
バナー未選択でもそちらからは出る。
